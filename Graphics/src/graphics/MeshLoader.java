...
public class MeshLoader {
	...
	public static final MeshLoader loader = new MeshLoader();
	...
	private final HashMap<File, Mesh> data = new HashMap<>();
	...
	/**
	 * Returns the model if its loaded already, otherwise its parsed from the .obj file.
	 * 
	 * @param s
	 * @return
	 */
	public Mesh getMesh(File f, int mode){
		shadingMode = mode;
		Mesh m = data.get(f);
		if(m == null){
			try {
	...
	public Mesh getMesh(File s){
		return getMesh(s, SHADING_PHONG);
	}

	private Mesh readMesh(File file) throws FileNotFoundException{
	...
	private void readLine(String line){
		...
		if(line.startsWith("f "))
                ...
                {
                    Logger.W("ModelLoader: line not parsed: "+line);
                }
	...
	private void readImport(String line){
		...
		s = toJavaPath(s);
		try {
			MaterialLoader.loader.importLib(new File(s));
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
	}
	...
	private void readUseMTL(String line){
		selMat = MaterialLoader.loader.getMaterial(new File(splitAtFirstSpace(line)));
	}
